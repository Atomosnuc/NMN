...
const Swap = () => {
  ...
  const fetchLiveQuote = useCallback(async (targetInputIndex, targetOutputIndex, targetAmount) => {
  ...
  const switchTokens = () => {
    // Swap indices
    dispatch(setSelectedTokenA(outputIndex))
    dispatch(setSelectedTokenB(inputIndex))
  ...
  },[inputIndex,outputIndex,poolData])
  ...
  }, [fetchLiveQuote, updatePrice, inputIndex, outputIndex, inputAmount])
          ...
          <Form onSubmit={swapHandler} style={{ maxWidth: '450px', margin: '30px auto' }}>
              ...
              <InputGroup>
                ...
                >
                  {symbols.map((symbol, index) => (
                    <DropdownItem key={index} onClick={() => { dispatch(setSelectedTokenA(index)); setInputAmount(''); setOutputAmount(''); setEstimatedFee('0'); setEstimatedSlippage('0'); }}>
                      {symbol}
                    </DropdownItem>
                  ))}
                </DropdownButton>
            ...
            <Row className='my-4'>
              ...
              <InputGroup>
                ...
                >
                  {symbols.map((symbol, index) => (
                    <DropdownItem key={index} onClick={() => { dispatch(setSelectedTokenB(index)); setInputAmount(''); setOutputAmount(''); setEstimatedFee('0'); setEstimatedSlippage('0'); }}>
                      {symbol}
                    </DropdownItem>
                  ))}
                </DropdownButton>
